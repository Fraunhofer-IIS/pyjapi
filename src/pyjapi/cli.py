...
def format_completer(ctx, args, incomplete):
    return [click.shell_completion.CompletionItem(fmt_name, help=fmt['desc']) for fmt_name, fmt in util.FORMATS.items() if not incomplete or fmt_name.startswith(incomplete)]
